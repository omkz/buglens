...
def _truncate(value: str, maximum: int) -> str:
    if len(value) <= maximum:
        return value
    notice = "\n\n_Section truncated by BugLens._"
...
def _truncate_code(value: str, maximum: int) -> str:
    if len(value) <= maximum:
        return value
    notice = "\n# ... generated test truncated by BugLens ..."
    return value[: max(0, maximum - len(notice))].rstrip() + notice
...
def _reproduction_result(status: str | None) -> str:
    if status == "reproduced":
        return (
            "**Reproduced**\n\nBugLens reproduced the reported failure during "
            "the browser reproduction attempt."
        )
    if status == "not_reproduced":
        return (
            "**Not reproduced**\n\nBugLens did not reproduce the reported failure "
            "during this attempt."
        )
    if status == "blocked":
        return (
            "**Blocked**\n\nBugLens could not make a reliable reproduction "
            "determination."
        )
    return "**Not attempted**\n\nBugLens did not run a browser reproduction attempt."
